...
import { LeagueStatus, mockLeagues } from '@/data/leagues'
...
export default async function Home() {
  ...
  const [archivedLeagues, activeLeagues] = Lodash.partition(
    leagues,
    (league) => league.status === LeagueStatus.Archived
  )
    ...
    <div className='grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-body)]'>
      <main className='flex flex-col gap-5 row-start-2 items-center sm:items-start w-[640px]'>
        {activeLeagues.map((league, index) => (
          <LeagueCard
            key={`${league.title}.${league.status}.${index}`}
            {...league}
          />
        ))}
      </main>
